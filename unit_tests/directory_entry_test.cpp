...
#ifndef ELIB_FILESYSTEM_V2_UNIT_TEST_PATH
# error ELIB_FILESYSTEM_V2_UNIT_TEST_PATH must be set to the directory of this file
# endif

using namespace elib::fs::v2;
...
BOOST_AUTO_TEST_CASE(status_test)
{
    ...
    {
        const directory_entry d(ELIB_FILESYSTEM_V2_UNIT_TEST_PATH, bad_st, symlink_st);
        file_status ret;
    ...
    {
        const directory_entry d(ELIB_FILESYSTEM_V2_UNIT_TEST_PATH, bad_st, bad_st);
        file_status ret;
        BOOST_CHECK_NO_THROW(ret = d.status());
        BOOST_CHECK(ret.type() == file_type::directory);
    }
